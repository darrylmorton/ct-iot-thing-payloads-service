...
class RoutesHelper:
    ...
    def assert_thing_payload(actual_result, expected_result):
        assert RoutesHelper.validate_uuid4(actual_result["id"]) is True
        assert actual_result["device_id"] == expected_result["device_id"]
        assert (
            actual_result["payload_timestamp"] == expected_result["payload_timestamp"]
        )
    ...
    def assert_thing_payloads(actual_result, expected_result):
        ...
        for index in range(len(actual_result) - 1):
            RoutesHelper.assert_thing_payload(
                actual_result[index], expected_result[index]
            )
